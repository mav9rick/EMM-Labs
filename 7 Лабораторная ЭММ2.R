...
t_max <- 1000  # Время для моделирования
...
simulate_ruin_process <- function(U0, c, lambda, mu, t_max) {
  ...
  while (TRUE) {
    ...
    t <- times[length(times)] + tau  ### Исправленная ошибка = переменная переменная t представляла 
                                      ### сумму всех предыдущих моментов , теперь представляет очередной момент времени
    if (t > t_max) {
      U <- U + c * (t_max - times[length(times)])  ### Была убрана sum(times) при расчёте U
      times <- c(times, t_max)
      capital <- c(capital, U)
      break
    }
...
res2 <- simulate_ruin_process(U0, c, 0.5, 3, t_max)  ### Исправил  lambda с 0.1 до 0.5 
...
num_simulations <- 5000  ### Увеличил количество симуляций с 1000 до 5000 для повышения точности
U0 <- 100
c <- 1
lambda <- 0.3
mu <- 3
...
cat("Проверка условия Лундберга: ", ruin_probability <= psi_theoretical, "\n")